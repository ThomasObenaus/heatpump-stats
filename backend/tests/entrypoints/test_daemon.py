...
class TestDaemonMain:
    ...
    def mock_settings_production(self):
        ...
        with patch('heatpump_stats.entrypoints.daemon.settings') as mock:
            ...
            mock.SHELLY_PASSWORD = "test_password"
    ...
    def mock_adapters(self):
        ...
        with patch('heatpump_stats.entrypoints.daemon.ShellyAdapter') as mock_shelly, \
    ...
    async def test_main_production_mode_initialization(
        ...
        with patch('heatpump_stats.entrypoints.daemon.asyncio.gather', side_effect=cancel_after_short_time):
            await main()

        # Verify production adapters were initialized
        mock_adapters['shelly'].assert_called_once_with(
            host="192.168.1.100",
    ...
    @pytest.mark.asyncio
    async def test_main_production_mode_viessmann_connection_failure(
        self,
        mock_settings_production,
        mock_adapters
    ):
    ...
    @pytest.mark.asyncio
    async def test_main_mock_mode_initialization(
        self,
        mock_settings_mock,
        mock_mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_collection_loops_execution(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_power_collection_error_handling(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_metrics_collection_error_handling(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_config_check_error_handling(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_collection_intervals(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_cancelled_error_handling(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
        ...
        with patch('heatpump_stats.entrypoints.daemon.asyncio.gather', side_effect=raise_cancelled):
            # Should not raise, just exit gracefully
            await main()

    @pytest.mark.asyncio
    async def test_main_mode_case_insensitive(
        self,
        mock_adapters,
        mock_mock_adapters,
        mock_collector_service
        ...
        with patch('heatpump_stats.entrypoints.daemon.settings') as mock_settings:
            ...
            mock_settings.SHELLY_PASSWORD = "test_password"
            ...
            mock_settings.INFLUXDB_TOKEN = "test_token"
    ...
    @pytest.mark.asyncio
    async def test_main_non_production_uses_mock(
        self,
        mock_mock_adapters,
        mock_collector_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_main_all_loops_run_concurrently(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_service_initialization_with_correct_adapters(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service
    ...
    @pytest.mark.asyncio
    async def test_main_logging_info_on_start(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service,
    ...
    @pytest.mark.asyncio
    async def test_main_logging_on_cancellation(
        self,
        mock_settings_production,
        mock_adapters,
        mock_collector_service,
