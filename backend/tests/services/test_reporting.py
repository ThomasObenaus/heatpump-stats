...
async def test_get_recent_history():
    # Arrange
    mock_repo = MagicMock()
    mock_repo.get_heat_pump_history = AsyncMock(return_value=[
        HeatPumpData(timestamp=datetime.now(timezone.utc), outside_temperature=10.0)
    ])
    mock_repo.get_power_history = AsyncMock(return_value=[
        PowerReading(timestamp=datetime.now(timezone.utc), power_watts=500.0)
    ])
